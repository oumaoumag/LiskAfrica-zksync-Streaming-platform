...
contract zkTune is Ownable {
    ...
    struct Artist {
        ...
        uint256 totalRoyalties; // Total royalties earned
        uint256 songsCount;     // Number of songs published
    }
    ...
    struct User {
        ...
        uint256 streamedCount; // Number of songs streamed
    ...
    struct Song {
        ...
        uint256 royaltyBalance;// Accumulated royalties
        uint256 nftPrice;     // Price of the NFT
        string genre;         // Genre of the song
        uint256 createdAt;    // Timestamp when song was added
    }
    ...
    // Mapping to track royalty payments
    mapping(address => uint256) public royaltyPayments;
    // Mapping to track user stream history with timestamps
    mapping(address => mapping(uint256 => uint256)) public userStreamTimestamps;
    ...
    function registerArtist(string memory _name, string memory _profileURI) external {
        ...
        artists[msg.sender] = Artist(_name, _profileURI, false, 0, 0);
        // Store artist details in the artistID mapping
        // ASSIGNMENT #2
        artistID[newArtistId] = Artist(_name, _profileURI, false, 0, 0);
    ...
    function registerUser(string memory _name, string memory _profileURI) external {
        ...
        users[msg.sender] = User(_name, _profileURI, 0, true);
    ...
    function addSong(string memory _title, string memory _audioURI, string memory _coverURI, uint256 _nftPrice) external onlyRegisteredArtist {
        ...
        // ASSIGNMENT #5
        songs[newSongId] = Song(newSongId, msg.sender, _title, _audioURI, _coverURI, 0, address(songNFT), 0, _nftPrice, "", block.timestamp);
